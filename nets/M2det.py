...
def TUM(stage, inputs, feature_size=256, name="TUM"):
    ...
    # 3,3,256
    c5 = keras.layers.Lambda(lambda x: tf.image.resize(x, size=size_buffer[4]), name=name + "_" + str(stage) + '_upsample_add5')(c5)
    ...
    # 5,5,256
    c4 = keras.layers.Lambda(lambda x: tf.image.resize(x, size=size_buffer[3]), name=name + "_" + str(stage) + '_upsample_add4')(c4)
    ...
    # 10,10,256
    c3 = keras.layers.Lambda(lambda x: tf.image.resize(x, size=size_buffer[2]), name=name + "_" + str(stage) + '_upsample_add3')(c3)
    ...
    # 20,20,256
    c2 = keras.layers.Lambda(lambda x: tf.image.resize(x, size=size_buffer[1]), name=name + "_" + str(stage) + '_upsample_add2')(c2)
    ...
    # 40,40,256
    c1 = keras.layers.Lambda(lambda x: tf.image.resize(x, size=size_buffer[0]), name=name + "_" + str(stage) + '_upsample_add1')(c1)
